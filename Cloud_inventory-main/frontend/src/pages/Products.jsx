...
const empty = { name: "", sku: "", category: "general", stock_qty: 0, reorder_threshold: 10, unit_price: "" };
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-2">
        ...
        <Input type="number" min={0} data-testid="product-form-price" value={value.unit_price ?? ""} onChange={set("unit_price")} />
      </div>
    </div>
  );
}
...
export default function Products() {
  ...
  const create = async () => {
    ...
    try {
      const payload = { ...form, unit_price: form.unit_price === "" ? null : Number(form.unit_price) };
                ...
                <TableBody>
                  {products.map((p) => (
                    <TableRow key={p.id} data-testid={`product-row-${p.id}`}>
                      ...
                      <TableCell className="text-right">{p.unit_price != null ? `$${p.unit_price.toLocaleString()}` : "—"}</TableCell>
                      ...
                      <TableCell className="text-right space-x-1">
                        <Button
                          ...
                          variant="ghost"
                          size="icon"
                          onClick={() => { setEditing(p); setForm({ name: p.name, sku: p.sku, category: p.category, stock_qty: p.stock_qty, reorder_threshold: p.reorder_threshold, unit_price: p.unit_price ?? "" }); }}
                        >
